...
export class GeolocationControl extends Control {
        ...
        switch (accuracy) {
            ...
            case GeoAccuracy.LOW:
                if (lastAccuraciesGrade === GeoAccuracy.UNKNOWN) {
                    ...
                    communicates.showErrorTemporary('Wykryto problemy z geolokalizacją');
                }
                break;
    ...
    static getControl() {
        return map.realMap.getControls().getArray().find(instance => instance instanceof GeolocationControl).element
    }
    static showButton() {
